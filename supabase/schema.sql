...
-- ── 15. ESTOQUE (materiais + histórico de movimentações) ──────
...
CREATE POLICY "iso_estoque" ON public.estoque FOR ALL
  USING (clinic_id = public.minha_clinica()) WITH CHECK (clinic_id = public.minha_clinica());

CREATE POLICY "iso_estoque_movimentacoes" ON public.estoque_movimentacoes FOR ALL
  USING (clinic_id = public.minha_clinica()) WITH CHECK (clinic_id = public.minha_clinica());
...
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.estoque WHERE id = NEW.estoque_id AND clinic_id = NEW.clinic_id) THEN
...
-- o histórico é a fonte única da verdade do saldo atual.
...
ALTER TABLE public.estoque ADD CONSTRAINT estoque_status_valido CHECK (status IN ('ativo','inativo'));
ALTER TABLE public.estoque_movimentacoes ADD CONSTRAINT estoque_mov_tipo_valido CHECK (tipo IN ('entrada','saida','ajuste'));
ALTER TABLE public.estoque_movimentacoes ADD CONSTRAINT estoque_mov_quantidade_positiva CHECK (quantidade > 0);
